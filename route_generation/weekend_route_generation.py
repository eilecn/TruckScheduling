...
for i in range(len(groups)):
    ...
    for j in range(0, len(groups[i])):
         ...
         first_store_demand = round(df.iat[j, df.columns.get_loc('SaturdayDemand')])
         ...
         for k in range(0, len(groups[i])):
            ...
            if (k != j):
                ...
                second_store_demand = round(df.iat[k, df.columns.get_loc('SaturdayDemand')])
                ...
                for l in range(0, len(groups[i])):
                    if (l != k and l != j):
                        ...
                        third_store_demand = round(df.iat[l, df.columns.get_loc('SaturdayDemand')])
